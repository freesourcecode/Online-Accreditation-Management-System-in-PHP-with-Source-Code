...
function doRequestDocuments()
{
	...
	if (isset($_POST['Document'])==''){
		...
		for($i=0;$i<$key;$i++){
			...
			$requestDocument->RequestDocuments = $document[$i];
			...
			$schoolRequest->RequestAccepted = 1;
			...
			$sql = "INSERT INTO `tblnotification`(`CreatedBy`,`SendTo`,`ForeignID`, `NotificationMessage`, `Category`, `AlreadyViewed`) VALUES ('".$_SESSION['COMPANY_USERID']."','{$schoolID}','{$requestID}','The agency ".$_SESSION['COMPANYNAME']." is Requesting the following documents','Requesting Documents',0)";
			$mydb->setQuery($sql);
			// $mydb->executeQuery();
...
function doCancel()
{
	...
	# code...
			...
			$schoolRequest->DeclinedDate = DATE('Y-m-d H:i:s');
			$schoolRequest->update($requestID);
